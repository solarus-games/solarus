...
-- A fairy appears on the map: set its movement
...
function event_obtaining(variant, savegame_variable)
  ...
  if sol.game.equipment_has_item("bottle_1")
    ...
    sol.map.dialog_start("_found_fairy")
...
function event_dialog_finished(first_message_id, answer)
  ...
  if first_message_id == "_found_fairy" then
    ...
    if answer ~= 1 then
      ...
      if first_empty_bottle == "" then
        -- no empty bottle
        sol.map.dialog_start("_found_fairy.no_empty_bottle")
  ...
  elseif first_message_id == "_found_fairy.no_empty_bottle" then
    sol.game.equipment_add_life(7 * 4)
  end
end
