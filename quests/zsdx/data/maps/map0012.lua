...
function event_map_started(destination_point)
  ...
  if sol.game.savegame_get_boolean(37) then -- if the Lyriann cave is finished
    sol.map.door_set_open("door", sol.game.equipment_is_dungeon_finished(1)) -- don't allow the player to obtain the bow until the first dungeon is finished
  end
end
...
function event_npc_dialog(npc_name)
  ...
  elseif sol.game.equipment_is_dungeon_finished(1) and not has_obtained_bow() then -- glove
