...
function event_map_started(destination_point_name)
  ...
  if sol.map.get_game():get_boolean(127) then
  ...
  if sol.map.get_game():get_boolean(128) then
  ...
  -- north chest
  if sol.map.get_game():get_boolean(950) then
...
function event_switch_activated(switch_name)
  ...
  elseif switch_name == "se_water_switch" and not sol.map.get_game():get_boolean(128) then
    sol.map.camera_move(912, 896, 250, remove_se_water, 1000, 3500)
  elseif switch_name == "1f_n_water_switch" and not sol.map.get_game():get_boolean(131) then
    remove_1f_n_water()
  elseif switch_name == "1f_e_water_switch_1"
      and sol.map.switch_is_activated("1f_e_water_switch_2")
      and not sol.map.get_game():get_boolean(122) then
    remove_1f_e_water()
...
function open_barrier()
  sol.audio.play_sound("secret")
  sol.map.tile_set_enabled("barrier", false)
  sol.map.get_game():set_boolean(127, true)
end
...
function remove_se_water_6()
  ...
  sol.map.get_game():set_boolean(128, true)
  sol.audio.play_sound("secret")
end
...
function remove_1f_n_water()
  ...
  sol.map.get_game():set_boolean(131, true)
end
...
function remove_1f_e_water()
  ...
  sol.audio.play_sound("water_drain")
  sol.map.dialog_start("dungeon_3.water_drained_somewhere")
  sol.map.get_game():set_boolean(122, true)
end
