...
-- puzzle A
local puzzle_a_switches = map:find_all("puzzle_a_switch")
local puzzle_a_red_tiles = map:find_all("puzzle_a_red_tile")
local puzzle_a_green_tiles = map:find_all("puzzle_a_green_tile")

-- puzzle B
local puzzle_b_switches = map:find_all("puzzle_b_switch")
...
local nw_switches = map:find_all("nw_switch")
local s_door_enemies = map:find_all("s_door_enemy")
local e_room_enemies = map:find_all("e_room_enemy")
local c_room_switches = map:find_all("c_room_switch")
local close_c_door_sensors = map:find_all("close_c_doors_sensor")
local reset_solid_ground_sensors = map:find_all("reset_solid_ground_sensor")
local hidden_enemies = map:create_list{hidden_enemy_1, hidden_enemy_2}
-- same result as: local hidden_enemies = map:find_all("hidden_enemy")
...
function close_w_room_sensor:on_collision_hero()
  ...
  if w_room_door:is_open()
    ...
    function w_room_enemy_killed()
      if #map:find("w_room_enemy") == 0 then
	sol.audio.play_sound("chest_appears")
...
function puzzle_a_switches:on_activated()
  ...
  if index then
    ...
    for _, v in ipairs(to_change[index]) do
      local red_tile = map:get("puzzle_a_red_switch_" .. v)
      local green_tile = map:get("puzzle_a_green_switch_" .. v)
    ...
    local success = true
    for i = 1, 6 do
      local red_tile = map:get("puzzle_a_red_switch_" .. i)
