Solarus is an open-source Zelda-like game engine written in C++.
This engine is used by our Zelda fangames.
The Git repository of Solarus includes several subprojects:
- Solarus: the C++ engine,
- Zelda: Mystery of Solarus DX (or ZSDX for short): our real game (still under development),
- Zelda: Mystery of Solarus XD (or ZSXD for short): a humorous game released on April 1st, 2011,
- Solarus Quest Editor: an editor for creating quests, written in Java (still under development)

See our development blog (http://www.solarus-engine.org) to get more information and documentation
or download the demo.
The compilation and installation instructions can be found
in the install.txt file or at http://www.solarus-engine.org/source-code/compilation-instructions.

