...
function map:on_started()
  ...
  if game:get_value("blacksmith") == 1 then 
    blacksmith:remove(); metal:remove(); steam:remove()
  end
end
...
function map:dialog_with_blacksmith()
  ...
  if not var then var = 0; game:set_value("blacksmith", 0) end
  if var == 0 then -- We are before the battle event.
    ...
    game:start_dialog("blacksmith.2", function(answer)
      if answer == 2 then game:start_dialog("blacksmith.3") -- Refuse battle.
      else game:start_dialog("blacksmith.4", function() -- Accept battle.
        game:set_value("blacksmith", 1)
        game:get_hero():freeze()
        sol.timer.stop_all(blacksmith)
        sprite = blacksmith:get_sprite()
        sprite:set_direction(1)
        sprite:set_animation("walking")
        local m = sol.movement.create("straight")
        m:set_angle(math.pi/2); m:set_speed(30)
        m:start(blacksmith)
        function m:on_obstacle_reached()
          blacksmith:remove() -- Destroy blacksmith.
          game:get_hero():unfreeze() -- Unfreeze hero.
        end
      end) end
    end)
    end)
