...
void Camera::update() {
  ...
  if (is_fixed_on_hero()) {
    ...
    if (map_location.get_width() < SOLARUS_GAME_WIDTH) {
      ...
      x = std::min(std::max(x - SOLARUS_GAME_WIDTH_MIDDLE, 0),
          map_location.get_width() - SOLARUS_GAME_WIDTH);
    }

    if (map_location.get_height() < SOLARUS_GAME_HEIGHT) {
      y = (map_location.get_height() - SOLARUS_GAME_HEIGHT) / 2;
    }
    else {
      y = std::min(std::max(y - SOLARUS_GAME_HEIGHT_MIDDLE, 0),
          map_location.get_height() - SOLARUS_GAME_HEIGHT);
    }
  }
  else if (movement != NULL) {
    movement->update();
    x = movement->get_x() - SOLARUS_GAME_WIDTH_MIDDLE;
    y = movement->get_y() - SOLARUS_GAME_HEIGHT_MIDDLE;
...
void Camera::move(int target_x, int target_y) {
  ...
  const Rectangle& map_location = map.get_location();
  target_x = std::min(std::max(target_x, SOLARUS_GAME_WIDTH_MIDDLE),
      map_location.get_width() - SOLARUS_GAME_WIDTH_MIDDLE);
  target_y = std::min(std::max(target_y, SOLARUS_GAME_HEIGHT_MIDDLE),
      map_location.get_height() - SOLARUS_GAME_HEIGHT_MIDDLE);

  movement = new TargetMovement(target_x, target_y, speed);
  movement->set_xy(position.get_x() + SOLARUS_GAME_WIDTH_MIDDLE,
      position.get_y() + SOLARUS_GAME_HEIGHT_MIDDLE);
