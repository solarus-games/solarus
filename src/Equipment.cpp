...
/**
 * @brief Notifies the item scripts that a dialog box has just finished.
 *
 ...
 */
void Equipment::notify_dialog_finished(const MessageId &first_message_id, int answer) {

  std::map<std::string, ItemScript*>::const_iterator it;
  for (it = item_scripts.begin(); it != item_scripts.end(); it++) {
    it->second->event_dialog_finished(first_message_id, answer);
  }
}
