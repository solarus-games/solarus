...
void PapillosaurKing::update() {
  ...
  if (is_in_normal_state() && now >= next_egg_phase_date - 500) {
    ...
    std::list<MapEntity*> sons = get_entities().get_entities_with_prefix(ENEMY, get_name() + "_minillosaur");
	...
	MapEntity *egg = create(get_game(), MINILLOSAUR, "minillosaur_egg",
	    RANK_NORMAL, -1, get_name() + "_minillosaur",
