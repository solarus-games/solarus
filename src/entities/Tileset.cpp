...
int Tileset::l_background_color(lua_State* l) {
  ...
  lua_rawgeti(l, 1, 2);
  lua_rawgeti(l, 1, 3);
  Color color(LuaContext::check_integer(l, -3),
    LuaContext::check_integer(l, -2),
    LuaContext::check_integer(l, -1));
  lua_pop(l, 3);
...
int Tileset::l_tile_pattern(lua_State* l) {
  ...
  while (lua_next(l, 1) != 0) {
    ...
    if (key == "id") {
      id = LuaContext::check_integer(l, 3);
    }
    else if (key == "ground") {
      ground = LuaContext::check_enum<Obstacle>(l, 3, ground_names);
    }
    else if (key == "default_layer") {
      default_layer = LuaContext::check_integer(l, 3);
    }
    else if (key == "x") {
      if (lua_isnumber(l, 3)) {
        // Single frame.
        x[0] = LuaContext::check_integer(l, 3);
        i = 1;
      }
      else {
        // Multi-frame.
        ...
        while (lua_next(l, 3) != 0 && i < 4) {
          x[i] = LuaContext::check_integer(l, 5);
          ++i;
          lua_pop(l, 1);
        }
      }
      ...
      if (lua_isnumber(l, 3)) {
        // Single frame.
        y[0] = LuaContext::check_integer(l, 3);
        j = 1;
      }
      else {
        // Multi-frame.
        ...
        while (lua_next(l, 3) != 0 && j < 4) {
          y[j] = LuaContext::check_integer(l, 5);
          ++j;
          lua_pop(l, 1);
        }
      }
      ...
      width = LuaContext::check_integer(l, 3);
    }
    else if (key == "height") {
      height = LuaContext::check_integer(l, 3);
    }
