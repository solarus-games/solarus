-- This module reads a quest properties file with the format of Solarus 1.2
-- (quest.dat) and converts it into the format of Solarus 1.3.
...
function converter.convert(quest_path)
  ...
  function quest(p)
    ...
    if not p.solarus_version:match("^1.2%.?") then
      error("This is not a Solarus 1.2 quest: the detected version is " .. p.solarus_version)
    end
  ...
  output_file:write("  solarus_version = \"1.3\",\n")
